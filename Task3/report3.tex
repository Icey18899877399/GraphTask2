...
从视觉上看，解码图像与原图完全一致。图~\ref{fig:diff} 给出了 $I_s$ 与 $I_d$ 的像素值联合密度图，所有点严格分布在 $y=x$ 对角线上，证实 LZW 编解码为严格无损恢复。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\textwidth]{figures/fig_difference.png}
  \caption{$I_s$ 与 $I_d$ 像素值相关性密度图。所有像素对严格落在 $y=x$ 对角线上，颜色表示该灰度级的像素密度，验证了无损重建。}
  \label{fig:diff}
\end{figure}
