...
\begin{figure}[H]
  ...
  \caption{原图 $I_s$ 与由 \texttt{C.dat} 解码得到的图像 $I_d$}
\end{figure}

从视觉上看，解码图像与原图一致。进一步的逐像素验证见图 \ref{fig:diff}，差分像素数为 0，说明本实验实现的 LZW 编解码过程为无损恢复。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fig_difference.png}
  \caption{原图与解码图的差分验证}
  \label{fig:diff}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\textwidth]{figures/fig_histogram_source.png}
  \caption{原图 $I_s$ 的归一化灰度直方图}
\end{figure}

原图灰度主要集中在中低灰度区间，同时覆盖了较宽的灰度范围。该分布决定了图像熵较高，但局部像素序列仍存在可被 LZW 字典利用的重复结构。

\subsection{bpp、熵与同一性验证}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{figures/fig_metrics.png}
  \caption{LZW 编码 bpp 与原图熵对比}
\end{figure}
